...
void RemapWidget::startRemapping(uint actionIndex) {
	...
	// Store current action's mapping info for VKB to highlight/block keys
	ConfMan.setInt("remap_current_keycode", -1);
	ConfMan.setInt("remap_current_mouse", -1);
	Array<HardwareInput> currentMapping = _remapKeymap->getActionMapping(_remapAction);
	if (!currentMapping.empty()) {
		const HardwareInput &hw = currentMapping[0];
		if (hw.type == kHardwareInputTypeKeyboard) {
			ConfMan.setInt("remap_current_keycode", (int)hw.key.keycode);
		} else if (hw.type == kHardwareInputTypeMouse) {
			ConfMan.setInt("remap_current_mouse", (int)hw.inputCode);
		}
	}

	ConfMan.setInt("remap_active", 1);
...
void RemapWidget::stopRemapping() {
	...
	// Clean up VKB context
	ConfMan.setInt("remap_active", 0);
	ConfMan.setInt("remap_current_keycode", -1);
	ConfMan.setInt("remap_current_mouse", -1);
	ConfMan.setInt("remap_confirm_keycode", -1);
	ConfMan.setInt("remap_confirm_mouse", -1);
	ConfMan.setInt("remap_clear_request", 0);
	ConfMan.setInt("remap_cancel_request", 0);
...
void RemapWidget::handleTickle() {
	...
	// If VKB confirmed a key selection (via ConfMan)
	if (_remapInputWatcher->isWatching() && _remapKeymap && _remapAction &&
	    ConfMan.hasKey("remap_confirm_keycode") && ConfMan.getInt("remap_confirm_keycode") != -1) {
		int keycode = ConfMan.getInt("remap_confirm_keycode");
		ConfMan.setInt("remap_confirm_keycode", -1);

		// Create a synthetic event to look up the HardwareInput
		Event syntheticEvent;
		syntheticEvent.type = EVENT_KEYUP;
		syntheticEvent.kbd.keycode = (KeyCode)keycode;
		syntheticEvent.kbd.ascii = 0;
		syntheticEvent.kbd.flags = 0;

		Keymapper *keymapper = g_system->getEventManager()->getKeymapper();
		HardwareInput hwInput = keymapper->findHardwareInput(syntheticEvent);
		if (hwInput.type != kHardwareInputTypeInvalid) {
			_remapKeymap->registerMapping(_remapAction, hwInput);
			_changes = true;
		}
		stopRemapping();
	}

	// If VKB confirmed a mouse button selection (via ConfMan)
	if (_remapInputWatcher->isWatching() && _remapKeymap && _remapAction &&
	    ConfMan.hasKey("remap_confirm_mouse") && ConfMan.getInt("remap_confirm_mouse") != -1) {
		int mouseBtn = ConfMan.getInt("remap_confirm_mouse");
		ConfMan.setInt("remap_confirm_mouse", -1);

		Event syntheticEvent;
		if (mouseBtn == 0) {
			syntheticEvent.type = EVENT_LBUTTONUP;
		} else if (mouseBtn == 1) {
			syntheticEvent.type = EVENT_RBUTTONUP;
		} else {
			syntheticEvent.type = EVENT_MBUTTONUP;
		}
		syntheticEvent.mouse.x = 0;
		syntheticEvent.mouse.y = 0;
	...
	// If VKB requested a clear mapping
	if (_remapInputWatcher->isWatching() && _remapKeymap && _remapAction &&
	    ConfMan.hasKey("remap_clear_request") && ConfMan.getInt("remap_clear_request") == 1) {
		ConfMan.setInt("remap_clear_request", 0);
		_remapKeymap->unregisterMapping(_remapAction);
		_changes = true;
		stopRemapping();
	}

	// If VKB was cancelled (Esc pressed)
	if (_remapInputWatcher->isWatching() &&
	    ConfMan.hasKey("remap_cancel_request") && ConfMan.getInt("remap_cancel_request") == 1) {
		ConfMan.setInt("remap_cancel_request", 0);
		stopRemapping();
	}
