...
void InputWatcher::startWatching() {
	...
	_watching = true;
	_watchStartTime = g_system->getMillis();
}
...
bool InputWatcher::notifyEvent(const Event &event) {
	...
	// Grace period: ignore inputs shortly after startWatching() to avoid
	// capturing the key/button release that triggered the remap action
	if (g_system->getMillis() - _watchStartTime < kGracePeriodMs)
		return true;
