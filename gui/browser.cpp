...
void BrowserDialog::handleCommand(CommandSender *sender, uint32 cmd, uint32 data) {
	...
	case kListItemDoubleClickedCmd:
		{
			int sel = _fileList->getSelected();
			if (sel >= 0 && sel < (int)_nodeContent.size()) {
				if (_nodeContent[sel].isDirectory()) {
					_node = _nodeContent[sel];
					updateListing();
				} else if (!_isDirBrowser) {
					_choice = _nodeContent[sel];
					setResult(1);
					close();
				}
			}
	...
	case kListSelectionChangedCmd:
		...
		{
			int sel = _fileList->getSelected();
			if (sel >= 0 && sel < (int)_nodeContent.size() && _isDirBrowser && !_nodeContent[sel].isDirectory())
